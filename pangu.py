...
# coding: utf-8
"""
Vendored pangu.py 4.0.6.1.

CapsWriter keeps this copy because upstream treats Chinese smart double quotes
as brackets and inserts spaces around them, e.g. 中文 “引号” 文.  For Chinese
dictation output, those quotes should stay attached to adjacent CJK text while
the text inside the quotes still receives normal CJK/ASCII spacing.
"""
...
__version__ = "4.0.6.1-capswriter.1"
...
CONVERT_TO_FULLWIDTH_CJK_SYMBOLS_CJK = re.compile(
    "([{CJK}])([ ]*(?:[\\:]+|\\.)[ ]*)([{CJK}])".format(CJK=CJK)
)
CONVERT_TO_FULLWIDTH_CJK_SYMBOLS = re.compile(
    "([{CJK}])[ ]*([~\\!;,\\?]+)[ ]*".format(CJK=CJK)
)
...
CJK_LEFT_SMART_DOUBLE_QUOTE = re.compile("([{CJK}])[ ]+(\u201c)".format(CJK=CJK))
RIGHT_SMART_DOUBLE_QUOTE_CJK = re.compile("(\u201d)[ ]+([{CJK}])".format(CJK=CJK))
...
def spacing(text):
    ...
    while matched:
        ...
        new_text = "".join(
            (
                new_text[: start + 1],
                convert_to_fullwidth(new_text[start + 1 : end - 1]),
                new_text[end - 1 :],
            )
        )
        matched = CONVERT_TO_FULLWIDTH_CJK_SYMBOLS_CJK.search(new_text)
    ...
    while matched:
        ...
        new_text = "".join(
            (
                new_text[: start + 1].strip(),
                convert_to_fullwidth(new_text[start + 1 : end]),
                new_text[end:].strip(),
            )
        )
        matched = CONVERT_TO_FULLWIDTH_CJK_SYMBOLS.search(new_text)
    ...
    new_text = CJK_LEFT_SMART_DOUBLE_QUOTE.sub(r"\1\2", new_text)
    new_text = RIGHT_SMART_DOUBLE_QUOTE_CJK.sub(r"\1\2", new_text)
...
def cli(args=None):
    ...
    parser.add_argument(
        ...
        action="store",
        type=str,
        help="the text or file path to apply spacing",
    )
